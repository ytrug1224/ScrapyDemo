...
from ItSpider.items import JobBoleArticleItem
...
class JobbleSpider(scrapy.Spider):
    ...
    def parse_detail_css(response):
        ...
        comments_nums = response.css("a[href='#article-comment']::text").extract_first()
        if comments_nums:
            match_fav = re.match(".*?(\d+).*", comments_nums)
            if match_fav:
                comments_nums = match_fav.group(1)
